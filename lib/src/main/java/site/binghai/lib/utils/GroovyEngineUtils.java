...
public class GroovyEngineUtils {
    private static Map<String, GroovyEngine> groovyEngineMap;
    ...
    /**
     * 只需输入逻辑即可，例如 result.ret = context.a * context.b;
     * 调用invoke方法得到结果Map中即可通过ret获取
     */
    public static GroovyEngine instanceGroovyEngine(String script) throws ScriptException {
        String md5 = MD5.encryption(script);
        GroovyEngine engine = groovyEngineMap.get(md5);
        if (engine != null) return engine;
        engine = new GroovyEngine(prefix + script + suffix, md5);
    ...
    public static class GroovyEngine {
        ...
        public GroovyEngine(String script, String md5) throws ScriptException {
